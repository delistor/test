...
class ChunkWriter:
    ...
    def _open_next(self):
        self._close()
        self.index += 1
        if self.index == 1 and self.chunk_bytes <= 0:
            # 单文件模式（chunk<=0 表示不分块）
    ...
    def finish(self):
        self._close()
        # 只有一个块时，直接命名为 <prefix>.txt
        if self.index == 1 and getattr(self, "path", "").endswith(".part001.txt"):
            single = self.path.replace(".part001.txt", ".txt")
            os.replace(self.path, single)
...
def pack(src_dir: str, out_prefix: str, chunk_mb: float) -> None:
    ...
    if writer.index > 1:
        print(f"完成: 打包 {count} 个文件, {writer.index} 个块, "
              f"共 {size/1024/1024:.2f} MB -> {out_prefix}.part001.txt 等")
    else:
        print(f"完成: 打包 {count} 个文件 -> {out_prefix}.txt ({size/1024/1024:.2f} MB)")
